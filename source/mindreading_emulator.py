...
class MindReadingEmulator(nn.Module):
    def __init__(self, config : MindReadingEmulatorConfig):
        super().__init__()
        self.config = config
    ...
    def generate(self, input_ids, max_new_tokens=512, num_beams=1, stop_on_two_eos=True) -> list:
    ...
    def evaluate(self, dataloader : DataLoader, ctx):
        ...
        for batch in tqdm.tqdm(dataloader):
            labels  = batch['labels_purecot_o'].to(device)
            input_ids_all  = batch['input_ids_purecot_o'].to(device)
            # Remove answer part
    ...
    def trainModel(self, train_handler : DatasetHandler, test_handler : DatasetHandler, limit : float) -> None:
        ...
        for batch in tqdm.tqdm(train_dataloader):
            ...
            pure_cot  = batch['labels_purecot_o'].to(device)
            input_w_nocot = batch['input_ids_purecot_o'].to(device)
